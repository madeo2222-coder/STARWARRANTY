...
export async function POST(req: Request) {
  try {
    ...
    if (!pdfRes.ok) {
      ...
      console.error("generate-warranty-invoice-pdf reminder failed:", errorText);
    ...
    const { data, error } = await resend.emails.send({
      ...
      attachments: [
        {
          filename: `warranty-invoice-reminder-${invoiceId}.pdf`,
          content: pdfBase64,
        },
      ],
    });
