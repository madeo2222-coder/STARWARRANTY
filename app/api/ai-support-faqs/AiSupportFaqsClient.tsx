...
function createEmptyForm(defaultProductCategory: string) {
  return {
    id: "",
...
export default function AiSupportFaqsClient({
  faqGroup,
  pageTitle,
  pageDescription,
  productCategories,
  ...
  const [form, setForm] = useState(createEmptyForm(defaultProductCategory));

  const [errorMessage, setErrorMessage] = useState("");
  const [successMessage, setSuccessMessage] = useState("");

  const isEditing = Boolean(form.id);
  const datalistId = `faq-product-categories-${faqGroup}`;

  useEffect(() => {
    setForm(createEmptyForm(defaultProductCategory));
  }, [defaultProductCategory]);

  useEffect(() => {
    void loadFaqs();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filterCategory, faqGroup]);
  ...
  async function loadFaqs() {
    ...
    try {
      ...
      params.set("faq_group", faqGroup);

      if (filterCategory) {
        params.set("product_category", filterCategory);
      }

      const res = await fetchWithAuth(`/api/ai-support-faqs?${params.toString()}`);
  ...
  function startEdit(faq: AiSupportFaq) {
    setForm({
      id: faq.id,
      product_category: faq.product_category || defaultProductCategory,
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        throw new Error(
          json.error ||
            (isEditing ? "FAQの更新に失敗しました" : "FAQの登録に失敗しました")
        );
      }
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Link
            href="/ai-support-faqs"
            className={`rounded-lg border px-4 py-2 text-sm hover:bg-gray-50 ${
              faqGroup === "housing" ? "bg-black text-white hover:bg-gray-800" : ""
            }`}
          >
            住宅設備用
          </Link>
          <Link
            href="/ai-support-faqs/appliance"
            className={`rounded-lg border px-4 py-2 text-sm hover:bg-gray-50 ${
              faqGroup === "appliance" ? "bg-black text-white hover:bg-gray-800" : ""
            }`}
          >
            家電用
          </Link>
          <Link
            href="/ai-support-faqs/solar"
            className={`rounded-lg border px-4 py-2 text-sm hover:bg-gray-50 ${
              faqGroup === "solar" ? "bg-black text-white hover:bg-gray-800" : ""
            }`}
          >
            太陽光用
          </Link>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <input
              ...
              className="w-full rounded-lg border px-3 py-2"
              placeholder="例：電源が入りません。どうしたらいいですか？"
            />
          </div>
